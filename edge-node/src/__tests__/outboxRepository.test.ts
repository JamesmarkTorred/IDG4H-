...
describe('outboxRepository', () => {
  ...
  function enqueueTestOperation() {
    return enqueueOutboxOperation({
      ...
      payload: { id: 'test-entity', nested: { values: [0, false, null, 'test'] } },
    });
  }
  ...
  test('creates a pending outbox operation', () => {
    ...
    expect(record.payload).toEqual({
      id: 'patient-test-001', firstName: 'Synthetic', lastName: 'Patient',
    });
  ...
  test('round-trips JSON payloads and keeps unique operation IDs separate from row IDs', () => {
    ...
    expect(new Set([first.id, first.operationId, second.id, second.operationId]).size).toBe(4);
    expect(findOutboxById(first.id)).toEqual(first);
    expect(findOutboxByOperationId(first.operationId)).toEqual(first);
    const stored = db.prepare<[string], { payload: string }>(
      'SELECT payload FROM outbox WHERE id = ?'
    ).get(first.id);
    expect(JSON.parse(stored!.payload)).toEqual(first.payload);
  });
  ...
  test('returns only due pending or failed operations in creation order, respecting the limit', () => {
    ...
    jest.setSystemTime(new Date('2026-09-03T08:00:03.000Z'));
    const due = enqueueTestOperation();
    markOutboxProcessing(due.id);
    markOutboxFailed(due.id, 'Retry due', '2026-09-03T08:00:10.000Z');
    jest.setSystemTime(new Date('2026-09-03T08:00:04.000Z'));
    const deferred = enqueueTestOperation();
    markOutboxProcessing(deferred.id);
    markOutboxFailed(deferred.id, 'Retry later', '2026-09-03T08:00:11.000Z');

    jest.setSystemTime(new Date('2026-09-03T08:00:10.000Z'));
    expect(findPendingOutbox().map((record) => record.id)).toEqual([pending.id, due.id]);
    expect(findPendingOutbox(1).map((record) => record.id)).toEqual([pending.id]);
    expect(findPendingOutbox(0)).toEqual([]);
    jest.setSystemTime(new Date('2026-09-03T08:00:11.000Z'));
    expect(findPendingOutbox().map((record) => record.id)).toEqual([pending.id, due.id, deferred.id]);
  });

  test('retries preserve operation identity and acknowledgment clears retry metadata', () => {
    const created = enqueueTestOperation();
    markOutboxProcessing(created.id);
    markOutboxFailed(created.id, 'Temporary failure', '2026-09-03T08:01:00.000Z');
    jest.setSystemTime(new Date('2026-09-03T08:01:00.000Z'));
    markOutboxProcessing(created.id);
    ...
    jest.setSystemTime(new Date('2026-09-03T08:01:01.000Z'));
    markOutboxAcknowledged(created.id);

    const acknowledged = findOutboxById(created.id)!;
    expect(acknowledged.operationId).toBe(created.operationId);
    expect(acknowledged.attemptCount).toBe(2);
    expect(acknowledged.status).toBe('acknowledged');
    expect(acknowledged.acknowledgedAt).toBe('2026-09-03T08:01:01.000Z');
    expect(acknowledged.updatedAt).toBe(acknowledged.acknowledgedAt);
  ...
  test('prevents processing an already processing or acknowledged operation', () => {
    const created = enqueueTestOperation();
    markOutboxProcessing(created.id);
    expect(() => markOutboxProcessing(created.id)).toThrow('could not be marked processing');
    expect(findOutboxById(created.id)?.attemptCount).toBe(1);
    markOutboxAcknowledged(created.id);
    expect(() => markOutboxProcessing(created.id)).toThrow('could not be marked processing');
    expect(findOutboxById(created.id)?.status).toBe('acknowledged');
    expect(findOutboxById(created.id)?.attemptCount).toBe(1);
  });
  ...
  test('returns undefined for missing identifiers and reports missing update targets', () => {
    ...
    expect(findOutboxByOperationId('missing')).toBeUndefined();
    expect(() => markOutboxProcessing('missing')).toThrow('could not be marked processing');
    expect(() => markOutboxAcknowledged('missing')).toThrow('could not be acknowledged');
    expect(() => markOutboxFailed('missing', 'Test error', '2026-09-03T08:01:00.000Z'))
  ...
  test('retains deletion payloads after the clinical record is removed, with no foreign keys', () => {
    const patient = createPatient({
      lastName: 'Synthetic', firstName: 'Patient', birthDate: '1990-01-01', sex: 'unknown',
    });
    const record = enqueueOutboxOperation({
      entityType: 'patient', entityId: patient.id, operationType: 'delete', payload: patient,
    });
    expect(db.pragma('foreign_key_list(outbox)')).toEqual([]);
  ...
  test('schema initialization preserves queued records and enforces its indexes and constraints', () => {
    ...
    initSchema();
    expect(findOutboxById(first.id)).toEqual(first);
    const indexes = db.prepare<[], { name: string }>('PRAGMA index_list(outbox)').all();
    expect(indexes.map((index) => index.name)).toEqual(expect.arrayContaining([
      'idx_outbox_status', 'idx_outbox_next_attempt', 'idx_outbox_entity', 'idx_outbox_created_at',
    ]));
    expect(() => db.prepare('UPDATE outbox SET operation_id = ? WHERE id = ?')
      .run(first.operationId, second.id)).toThrow(/UNIQUE constraint failed/);
    expect(() => db.prepare('UPDATE outbox SET entity_type = ? WHERE id = ?')
      .run('invalid', first.id)).toThrow(/CHECK constraint failed/);
    expect(() => db.prepare('UPDATE outbox SET operation_type = ? WHERE id = ?')
      .run('invalid', first.id)).toThrow(/CHECK constraint failed/);
    expect(() => db.prepare('UPDATE outbox SET status = ? WHERE id = ?')
      .run('invalid', first.id)).toThrow(/CHECK constraint failed/);
    expect(findOutboxById(first.id)).toEqual(first);
  });
  ...
  test('participates in an enclosing SQLite transaction without committing it independently', () => {
    ...
    expect(failingTransaction).toThrow('Synthetic transaction failure');
