...
import { initializeAuthData } from '../auth/schema';
...
describe('SQLite session persistence', () => {
  test('a valid session survives a database close and reopen', () => {
    ...
    try {
      ...
      firstDatabase.pragma(
        'foreign_keys = ON'
      );

      // Provision the test database from the
      // Prisma-managed schema.
      firstDatabase.exec(migrationSql);

      // Seed application-level auth data.
      initializeAuthData(firstDatabase);

      const firstRepository =
        new AuthRepository(firstDatabase);
      ...
      const roleId =
        firstRepository.createRole(
          'restart-test-role'
        );

      firstRepository.grantPermissionToRole(
        roleId,
        'patients:read'
      );

      firstRepository.assignRoleToUser(
        user.id,
        roleId
      );
