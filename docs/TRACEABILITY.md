...
| SSR is calculated from observed synchronization state | Required | `syncMetrics.ts`, `evaluate-sync.cjs` | Metric unit tests and isolated stable/adverse smoke runs | Instrumented; final study result not yet measured |
| DCI compares expected and Central canonical data | Required | Full patient-field snapshots in `evaluate-sync.cjs`; `syncMetrics.ts` | Metric mismatch/missing/duplicate tests and isolated smoke runs | Instrumented; final study result not yet measured |
...
| Retry, duplicate, and data-loss outcomes are observable | Required | Edge/Central operation correlation plus proxy fault counters | Packet-loss, intermittent, outage, and interruption smoke runs | Instrumented |
